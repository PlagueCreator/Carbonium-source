...
module.exports = (app) => {
	...
	//http://localhost:5595/account/api/public/account?accountId=SparkleBOT10&accountId=SparkleBOT11&accountId=SparkleBOT12&accountId=SparkleBOT13&accountId=SparkleBOT14&accountId=SparkleBOT15
	...
	app.get('/account/api/public/account/', (req, res) => {
		try {
			...
			if (`${req.query.accountId}`.startsWith("SparkleBOT")) { displayName = "SparkleBOT" }
			...
			req.query.accountId.forEach(accId => {
				var dn = accId
				if (`${accId}`.startsWith("SparkleBOT")) { dn = "SparkleBOT" }
				response.push({
					id: accId,
					displayName: dn,
					externalAuths: {}
